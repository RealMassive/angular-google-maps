...
MarkerClusterer.prototype.addToClosestCluster_ = function (newClusters, clustersTree, zoom) {
    ...
    for(j = 0; j < N; j++) {
        ...
        alpha = Math.atan(zoomLeveDistance[zoom]/6353) * 250;
        ...
        for(i = 0; i < clusters.length; i++) {
            ...
            if (center) {
                d = _self.distanceBetweenPoints_(center, newClusterCenter);
                if (d < distance) {
                    distance = d;
...
MarkerClusterer.prototype.updateClusters_ = function (markers) {
    ...
    for (zoom = this.maxZoomLevel ; zoom >= 0 ; zoom--) {
        diff = this.addToClosestCluster_(diff, this.zoomLevelsClusters[zoom], zoom);
    }
};
